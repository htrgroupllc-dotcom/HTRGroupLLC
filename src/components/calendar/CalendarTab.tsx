...
export default function CalendarTab({
  ...
  const onDropSlot = (dayIdx: number, slotIdx: number) => {

    if (!dragId) return;

    void reschedule(dragId, isoFromWeekSlot(weekStart, dayIdx, slotIdx));
  ...
  const EventCard = ({ ev, compact }: { ev: CalendarEvent; compact?: boolean }) => (
    ...
    <button
      ...
      onClick={() => { setSelected(ev); setMoveOpen(false); }}
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-4 overscroll-contain" style={{ WebkitOverflowScrolling: "touch" }}>
            ...
            {weekDays.map((d) => {
              ...
              return (
                ...
                <button
                  ...
                  className={`snap-start shrink-0 min-w-[52px] min-h-[56px] rounded-xl border text-center py-1.5 touch-manipulation ${active ? "border-blue-600 bg-blue-50 text-blue-700" : "border-stone-200 bg-white text-stone-600"}`}
                ...
                >
                  ...
                  {cnt > 0 && <div className="text-[9px] mt-0.5">{cnt}</div>}
          ...
          <div className="space-y-2 mt-2">
            ...
            {eventsForDay(mobileDay).length === 0 && !loading && (
              ...
              <div className="text-center text-stone-400 text-sm py-8">{labels.noEvents}</div>

            )}

            {eventsForDay(mobileDay).map((ev) => (
          ...
          <div className="min-w-[700px] border border-stone-200 rounded-lg bg-white overflow-hidden">
            ...
            <div className="grid grid-cols-8 border-b border-stone-200 bg-stone-50 text-[10px] font-semibold text-stone-500">
              ...
              {weekDays.map((d) => (
                ...
                <div key={d.toISOString()} className={`p-2 text-center ${isSameDay(d, houstonNow()) ? "text-blue-600" : ""}`}>
            ...
            <div className="relative" style={{ minHeight: TIME_SLOTS.length * 32 }}>

              {TIME_SLOTS.map((label, slotIdx) => (

                <div key={label} className="grid grid-cols-8 border-b border-stone-100 min-h-[32px]">

                  <div className="text-[9px] text-stone-400 p-1 pr-2 text-right border-r border-stone-100">{label}</div>
                  ...
                  {Array.from({ length: 7 }, (_, dayIdx) => (
                    ...
                    <div
                      ...
                      onDrop={() => onDropSlot(dayIdx, slotIdx)}
              ...
              {events.map((ev) => {
                ...
                const height = Math.max(1, slotEnd - slotStart) * 32;

                const top = slotStart * 32;
                ...
                return (
                  ...
                  <div
                    ...
                    className="absolute text-white text-[9px] rounded px-1 py-0.5 cursor-pointer overflow-hidden shadow-sm z-10 flex items-start gap-0.5"
              ...
              {getMonthGrid(anchor.getFullYear(), anchor.getMonth()).map((day) => {
                ...
                return (
                  ...
                  <button
                    ...
                    className={`min-h-[64px] md:min-h-[80px] border-b border-r border-stone-100 p-1 text-left touch-manipulation ${inMonth ? "" : "bg-stone-50/80"} ${isSameDay(day, houstonNow()) ? "ring-1 ring-inset ring-blue-400" : ""}`}

                    onClick={() => { setAnchor(day); setMobileDay(day); setView("week"); }}

                  >

                    <div className={`text-[10px] font-semibold mb-0.5 ${inMonth ? "text-stone-600" : "text-stone-300"}`}>
                    ...
                    {dayEvents.length > (isMobile ? 2 : 3) && (
                      ...
                      <div className="text-[8px] text-stone-400">+{dayEvents.length - (isMobile ? 2 : 3)}</div>
            ...
            {MONTHS_LONG.map((name, mi) => {
              ...
              return (
                ...
                <button
                  ...
                  className="border border-stone-200 rounded-lg bg-white p-2 text-left hover:border-blue-300 active:bg-stone-50 transition min-h-[44px] touch-manipulation"
                  ...
                  <div className="border border-stone-200 rounded-lg p-3 bg-stone-50">
                    ...
                    <div className="grid grid-cols-3 gap-1.5 max-h-[200px] overflow-y-auto">
                      ...
                      {TIME_SLOTS.map((slot, slotIdx) => {
                        const dayIdx = weekDays.findIndex((d) => isSameDay(d, isMobile ? mobileDay : new Date(selected.start_at)));
                        const safeDay = dayIdx >= 0 ? dayIdx : 0;
                        return (
                        <button
                          key={slot}
                          type="button"
                          onClick={() => {
                            void reschedule(selected.id, isoFromWeekSlot(weekStart, safeDay, slotIdx));
                          }}
